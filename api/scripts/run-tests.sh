...
run_unit_tests() {
    ...
    if ensure_gotestsum; then
        ...
        gotestsum --format testname -- -v -race -coverpkg=./internal/... -coverprofile=unit.out -covermode=atomic ./internal/...
    else
        log "Using go test"
        go test -v -race -coverpkg=./internal/... -coverprofile=unit.out -covermode=atomic ./internal/...
    fi
