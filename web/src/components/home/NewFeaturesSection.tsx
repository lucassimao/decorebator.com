      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {features.map((feature) => (
            <div
              key={feature.title}
              className={`rounded-2xl p-8 ${feature.bg} border ${feature.border} transform transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl`}
            >
              <div
                className={`h-16 w-16 ${feature.iconBg} mb-6 flex items-center justify-center rounded-2xl transition-transform duration-300`}
              >
                <i className={`${feature.icon} text-2xl text-white`}></i>
              </div>
              <h3 className="mb-3 text-xl font-bold">{feature.title}</h3>
              <p className="leading-relaxed text-[#636E72]">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
