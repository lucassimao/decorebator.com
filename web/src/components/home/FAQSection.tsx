    ...
    <section id="faq" className="bg-white py-20">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div className="mb-16 text-center">
          ...
          <p className="text-xl text-[#636E72]">{t('subtitle')}</p>
        </div>
        ...
        <div className="space-y-4">
          {faqs.map((faq) => (
            <div
              key={faq.id}
              className="overflow-hidden rounded-2xl border border-gray-200 transition-colors hover:border-orange-200"
            >
              <button
                className="flex w-full items-center justify-between px-6 py-4 text-left transition-colors hover:bg-orange-50"
                onClick={() => toggleFAQ(faq.id)}
              >
                <span className="text-lg font-semibold">{faq.question}</span>
                <i
                  className={`fas fa-chevron-down text-[#636E72] transition-transform ${openFAQ === faq.id ? 'rotate-180' : ''}`}
                ></i>
              </button>
              <div
                className={`px-6 transition-all duration-300 ease-in-out ${openFAQ === faq.id ? 'max-h-96 pb-4' : 'max-h-0 overflow-hidden'}`}
              >
                <p className="leading-relaxed text-[#636E72]">{faq.answer}</p>
              </div>
            </div>
          ))}
        </div>
