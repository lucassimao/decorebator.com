...
export async function getUserWordlists() {
    ...
    if (!response.ok) {
        const message = body?.error || Object.values(body?.validationErrors)?.[0] || DEFAULT_ERROR
...
export async function getWords(wordlistId: number): Promise<Word[]> {
    const endpoint = process.env.EXPO_PUBLIC_API_URL + `/wordlists/${wordlistId}/words`
    const authorization = await SecureStore.getItemAsync('authorization')
...
export async function addWord(dto: CreateWordDTO): Promise<void> {
    const endpoint = process.env.EXPO_PUBLIC_API_URL + `/wordlists/${dto.wordlistId}/words`
    const authorization = await SecureStore.getItemAsync('authorization')
...
export async function deleteWord(word: Word): Promise<void> {
    ...
    const endpoint = process.env.EXPO_PUBLIC_API_URL + `/wordlists/${wordlistId}/words/${wordId}`
    const authorization = await SecureStore.getItemAsync('authorization')
...
export async function newQuiz(wordlistId: number): Promise<Quiz> {
    const endpoint = process.env.EXPO_PUBLIC_API_URL + `/wordlists/${wordlistId}/quizzes`
    const authorization = await SecureStore.getItemAsync('authorization')
...
export async function answerQuiz(wordlistId:number,quizId: number, success:boolean): Promise<void> {
    const endpoint = process.env.EXPO_PUBLIC_API_URL + `/wordlists/${wordlistId}/quizzes`
    const authorization = await SecureStore.getItemAsync('authorization')
