...
import { ActivityIndicator, Button, Dialog, Portal, Snackbar, Text, useTheme } from 'react-native-paper';
...
const DeleteWordlistDialog = ({ onDismiss, wordlist }: Props) => {
    ...
    return (
        <Portal>
            ...
            <Dialog visible={true} onDismiss={cancel} dismissable dismissableBackButton>
                <Dialog.Title style={styles.title}>Add Word</Dialog.Title>
                <Dialog.Content>

                    {isPending ? <ActivityIndicator size={'large'} animating={true} theme={theme} /> :
                        <Text>Are your sure? {wordlist.name} will be removed altogether, including the words</Text>
                    }
                </Dialog.Content>
                <Dialog.Actions>
