...
- **Worker concurrency required an explicit database budget**: the audited configuration actually exposed 235 nominal River slots, not 226, against a 10-connection worker pool. MaxWorkers is a scheduling ceiling rather than a connection reservation, but the definition fetcher holds a transaction across sequential TTS and MinIO work. The revamp therefore caps that queue at 4, leaving at least 6 pool connections outside its blast radius; the resulting ceilings are 189 with the temporary legacy rollback surface and 179 in IAP-only mode. OpenAI calls and workers are now bounded and cancellable. Moving meaning-audio generation outside the definition transaction remains the durable fix and is required before increasing that queue or worker load.
...
| C | Worker DB pool = 10; definition concurrency is temporarily capped at 4 so its long TTS + MinIO transaction cannot monopolize the pool. Nominal River ceilings are 189 with legacy rollback queues and 179 in IAP-only mode; transaction extraction and measured capacity planning remain required before increasing load | `cmd/workers/main.go`, `river.go`, `definition_fetcher_worker.go` |
