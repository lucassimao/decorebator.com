...
Restore is provider refresh, not ownership claiming. Apple transactions and Google tokens must resolve through an existing exact account binding; the currently signed-in user cannot take over an unbound or differently bound purchase. Requests contain at most 20 evidence items and are rejected rather than truncated above that bound. Apple evidence is verified, reduced to `originalTransactionId`, deduplicated by lineage, and refreshed sequentially at most once per lineage. Server-known protected bindings are also refreshed so an incomplete SDK list does not strand an existing account purchase. Per-evidence results expose only the request index and `restored`, `unchanged`, `rejected`, or `retryable`; raw evidence is never echoed. Mixed results aggregate as `partial`, any unresolved transient work remains visible as pending/partial, and rejected evidence never collapses to `no_purchases`. Empty client evidence plus no known bindings is the only `no_purchases` result.

The authenticated API is `GET /subscription/iap/context?store=apple|google`, `POST /subscription/iap/apple/verify`, `POST /subscription/iap/google/verify`, and `POST /subscription/iap/restore`. Provider-calling routes use bounded bodies, a 25-second outer deadline, a per-user request budget, and one process-local in-flight operation per user. Production IAP requires configured Redis-backed limiting; nonproduction without Redis uses bounded local limiting and reports degraded mode, while a configured Redis outage fails closed with a retryable response.

`STORE_IAP_CLIENT_ENVIRONMENT` is selected only by the backend. A production deployment accepts and reads production evidence only and never probes Apple's sandbox host after a production miss. TestFlight/sandbox validation uses a separate nonproduction backend configured for `sandbox`; the production smoke transaction uses the production backend. Request bodies cannot choose an environment, and sandbox rows never contribute to production access or responses.
...
- `currentEntitlement` is the only positive premium-state input. It contains no internal record/user IDs, provider evidence, raw payload, or environment. It never encodes `pending`; mobile may display status, renewal, cancellation, and expiry but must not locally promote pending/failed purchases.
- `pending` is the sole pending-purchase representation. It may coexist with a different current active entitlement during a plan replacement, identifies only store, product, and start time, and never grants access. A verified Google purchase that still requires acknowledgement remains pending and cannot grant.
...
- `restore: null` is the only not-requested representation. After a restore request it is `restored`, `no_purchases`, `partial`, `pending`, or `failed`, with bounded per-evidence outcomes and aggregate counts. A successful empty restore is not an error; invalid, unknown, unbound, and foreign evidence all share one client-safe permanent error so the API is not an ownership oracle.
...
Google additionally requires the current purchase binding to be acknowledged. A verified active/grace row with `acknowledged=false` is durable pending state, not access. A unique evidence-free River retry starts after one minute with capped exponential backoff; a five-minute sweep of persisted unacknowledged bindings repairs an enqueue race, alerts begin at 24 hours, and exhaustion is reported no later than 60 hours, before Play's approximate 72-hour automatic-refund boundary.

While Stripe/RevenueCat coexist, effective access is additive: either a valid legacy plan or a valid exact-environment store entitlement grants premium. Store revocation/expiry cannot revoke a still-valid legacy plan. A request-scoped compatibility projection makes subscription limits, workers, quiz selection, analytics cache policy, error-report limits, and profile output agree without writing a fake monthly/annual value to `users`; the legacy branch is removed in Phase 4.
