...
query_file="$script_dir/activation-cohort-report.sql"
...
if ! command -v psql >/dev/null 2>&1; then
  echo "psql is required to run the read-only cohort report" >&2
  exit 1
fi

registry_json=$(jq -c '[.accounts[] | select(.excludeFromActivationReports == true) | .userId]' "$registry_file")
db_url="${DATABASE_URL:-}"
